...
- Use `open-gpui-devtools` for read-only local inspection: app-owned probes and feature-gated first-party adapters collect serializable snapshot envelopes, diagnostics, redaction summaries, and an optional GPUI inspector surface behind the `gpui` feature.
