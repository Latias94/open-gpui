...
- `open-gpui-devtools` diagnostics now include a stable `code`; replace `SnapshotDiagnostic { probe_id, message }` struct literals with `SnapshotDiagnostic::new(...)` or `SnapshotDiagnostic::collection_failed(...)`. `ProbeId::new(...)` now sanitizes ids at creation/deserialization, `SnapshotKind::as_label()` returns `Cow<'_, str>` while sanitizing custom labels for UI rows and JSON export, and `SnapshotNode::new`, `SnapshotNode::with_payload`, and `SnapshotRedactionSummary` now sanitize DTO output channels instead of preserving raw strings.
