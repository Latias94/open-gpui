...
- The UI framework authority-convergence changes are intentionally alias-free. Use the [Open GPUI v0.3 UI migration guide](docs/ui/migration-v0.3.md) for complete before/after examples and ownership rules.
- `open-gpui-form` now uses store-bound validation and submission tickets: `FormStore` is no longer `Clone`, async completion and submit completion return typed outcomes, ticketless finish APIs are removed, and active validation participates in derived form status and submission eligibility.
- `open-gpui` pointer capture is now explicitly window- and owner-scoped: retain and bind a `PointerCaptureHandle`, pass the initiating button to `Window::capture_pointer`, release through the same handle, use the routed-event-target query separately from physical hover, and replace `Window::remove_window()` with `Window::remove_window(cx)`.
- `open-gpui` makes renderer texture leasing a required `PlatformAtlas` contract. Third-party backends must implement `atlas_texture_lease_epoch`, atomic `acquire_atlas_texture_leases`, and matching `release_atlas_texture_leases` with exact `AtlasTextureInstanceId` generations; `AtlasTile` literals and host/shader ABI mirrors must add `texture_generation` plus zero `texture_generation_padding`. The former `Unsupported` defaults are removed because they compiled successfully while silently dropping retained glyph, icon, emoji, and image sprites.
- `open-gpui` adds `PlatformWindowHit::{ProvisionalPassThrough, NoInputPassThrough}` for exact-generation native routing through gated provisional and pointer-transparent windows. Update exhaustive matches and custom point-hit stacks to emit only a front-to-back pass-through prefix followed by one registered, opaque, or verified-open-desktop terminus; replace `PlatformWindowHit::is_provisional_pass_through()` with `is_pass_through()` only when both variants are valid, otherwise match `ProvisionalPassThrough` explicitly. Keep or return `PlatformWindowHitStack::Unavailable` when complete coverage cannot be proven.
- `open-gpui` removes `PlatformViewportCapabilities::live_window_move`, `PlatformViewportFlagCapabilities`, `App::viewport_flag_capabilities`, and the parallel unit/bool platform-window setters. Query the property-specific `Unsupported` / `CreationOnly` / `Live` matrix for the actual window kind and target display, dispatch typed `WindowMutationRequest` values, retain queued tickets when terminal results matter, and read committed bounds, state, and flags from `WindowPlatformFacts`.
- `open-gpui` removes the overloaded `WindowOptions::focus` field and live focus-on-appearing/click mutation helpers. Use creation-only `WindowOptions::focus_on_appearing` for first appearance, `WindowOptions::activation_policy` for lifetime activation and click focus, and an app-bound `WindowOptions::transient_for` token only when a native owner relationship is intentional.
- `open-gpui` removes the SVG-only `Transformation`, `TransformationMatrix`, and `with_transformation` APIs in favor of checked `SubtreeTransform` plus `ElementGeometry` and target-local event adapters; the supported contract is finite positive normal axis scale and finite translation, and SVG rotation has no replacement.
- `Window::record_prepaint_window_transaction` now delivers a window-bound `AcceptedFrameFence` to commit and discard callbacks instead of a bare revision. Use `AcceptedFrameFence::generation()` only for diagnostics; rejected candidates publish no fence and preserve the previously accepted authority.
- `WindowProvisionalSemanticsOutcome::{Pending, Committed, WindowTerminal}`, `WindowProvisionalSessionPhase::DestinationSemanticsCommitted`, and `committed_frame_generation()` are replaced by explicit accepted/submitted/rejected states and generation accessors. Provisional interaction now requires renderer `Submitted` evidence for the exact accepted destination frame; accepted, deferred, invalidated, or rejected frames remain gated.
- `open-gpui` `view_presentation_window::claim_batch` now assigns one lease generation to all newly claimed roots. `ClaimError::MixedBatchGenerations` rejects attempts to retroactively group roots that were claimed separately under incompatible generations; claim them together or keep separate batches.
- `WindowProvisionalRevealOutcome` now includes `Cancelled`. Exact provisional reveal owners must handle the new terminal outcome and use `Window::cancel_provisional_presentation` with `WindowProvisionalRevealCancellationOutcome` when cancellation must race a queued native reveal.
- `PlatformWindowCommand::RevealDeferredInitialPresentation { session_generation, presentation_generation }` now requires an exact `reveal_point: Point<DevicePixels>` payload; custom backends must preserve that point through native dispatch instead of deriving it from later pointer state or window bounds.
- `open-gpui` removes the rectangle-only `ContentMask`, `Window::with_content_mask`, and raw window-space mask injection APIs. Use checked child-local `SubtreeClip` declarations through `SubtreeClipExt::with_subtree_clip`; custom elements resolve a `PreparedSubtreeClip` during prepaint and re-enter that same token during paint.
- `open-gpui` deletes `Style::visibility`, generated Serde/schema `StyleRefinement::visibility`, `Visibility::{Visible, Hidden}`, `visibility_style_methods!` and its generated `.visible()` / `.invisible()` (`fn visible` / `fn invisible`) methods, and the separate `a11y_hidden` / `aria_hidden` ancestor paths in favor of `SubtreePresentation::{Visible, Inert, Hidden}`. `Inert` preserves layout and paint, `Hidden` preserves layout only, and both revoke input, focus, IME, overlay intent, and final accessibility membership without replaying stale interactions when restored.
- `open-gpui` replaces `ListState::scroll_to_reveal_item` and component-owned final reveal arithmetic with the window-owned `RevealTargetHandle` / `Window::bring_into_view` authority shared by applications, winning focus claims, and AccessKit. `open-gpui-ui-components` replaces `VirtualizedListScrollStrategy`, `VirtualizedListRevealTarget`, `VirtualizedListRevealResult`, and `scroll_target_for_key*` with `VirtualizedListMaterialization*` plus `VirtualizedList::bring_key_into_view`; `UniformListScrollHandle` keeps named direct-scroll methods while its tuple field and pending state types become private.
- `open-gpui` adds typed one-shot `focus_with_completion` / `blur_with_completion`, synchronous `committed_focus`, exact-leaf `on_focus_committed`, and subtree `on_focus_committed_in` APIs for final window-local focus authority independent of platform activation; `focused` remains the current intent query, `on_focus_in` retains active-window semantics, and `Window::blur` / `disable_focus` now require `cx`.
- `open-gpui-ui-core` renames `SemanticDescriptor::{with_hidden, hidden}` to `with_omit_accessibility_node` and `omit_accessibility_node` without aliases. The value omits only the descriptor's current semantic node, preserves descendant projection, and appears under the matching `omit_accessibility_node` key in strict DevTools payloads.
- Official overlays now share one per-window `WindowOverlayRuntime` for lifecycle, topmost input, modal focus, and restoration. `OverlayFocusTarget`, the public `FocusScopeRuntime` constructor, `OverlayLayerHost`, `gpui_relative_overlay_layer`, and family-local focus/dismissal tails are removed; `OverlayPresence::from_parts` is fallible, `FocusRestoreInput::ancestor_last_targets` is renamed, `WindowOverlayRuntime::set_inside_region` is replaced by checked `set_element_inside_region`, and custom adapters use the low-level window registration API only when an official component is insufficient.
- Menu path-key outputs from `MenuItemState` and the related Menu state/selection accessors now percent-encode `/` and `%` inside each segment; update persisted keys, selector fixtures, caches, and equality assertions.
- Accessibility semantics now derive from resolved component state and are proven in final AccessKit trees and real action dispatch. Static `ComponentA11yEvidence` and Gallery claim APIs are removed, field relations use composed `Field` controls, and Table row-sensitive APIs use exact `TableRowIdentity` / `TableSourceRowIdentity` values instead of treating a business `TableRowId` as unique.
- Live updates now use committed `Status` / `Alert` semantics or the bounded window announcement queue. `CommandStatusItem::{new, info, warning, error}` require a stable id, while `ResourceCollectionProjection::resolve` and `ResourceMutationProjection::resolve` require a caller-owned `ResourceAdapterNamespace`; query keys, mutation ids, and user content must not become status identity. `Window::announce` returning `Accepted` means the request entered the queue, not that it cannot be cleared by a later accessibility lifecycle boundary before publication.
- Official semantic controls replace raw `on_click` or selection-shaped callbacks with role-specific activation/change intents and typed `Activation` sources. `selected(...)` inputs now remain caller-owned, `default_*` builders seed adapter-owned state, item handlers override family fallbacks, and ambiguous duplicate values fail closed.
- Theme selection is no longer a public app-global `ThemeRuntime`: resolve an owned `ThemeContext` through app, window, and stable-id `ThemeScope` authority, and explicitly scope raw GPUI tooltips. `ThemeDefinition` v1 is also a complete payload containing colors plus typography, spacing, radius, elevation, density, and motion policy; the old color-only loader shape is rejected.
- Tree, VirtualizedList, Menu, ContextMenu, and open Listbox surfaces now use one private deterministic typeahead session. Removed `Listbox::typeahead_query` metadata and public `active(...)` builders have no session-state replacement; use model-owned queries for editable search and `default_active(...)` only to seed adapter-owned collection focus.
- `open-gpui-devtools` GPUI focus capture now reads real active-window authority through `GpuiRuntimeFocusSnapshot::from_window`; rendered-focus, opaque revisions, and focus scope/handle counts are optional and remain absent when the producer or an older imported payload cannot prove them.
- `open-gpui-docking` replaces render-local colors and `DockDropGuideStyle` with one complete immutable `DockVisualStyle`, the structural `DockDropGuideMetrics`, and a read-only per-surface or explicit-host `DockVisualStyleResolver`. Source drag visuals freeze their opening-generation style outside payload identity, while target guides resolve the destination host's current window or subtree context.
- `open-gpui-docking` now gives `DockSurface` one owner-backed committed revision stream and stable-item activation outcomes. Replace generic-notify persistence inference and `DockHost::focus_pane` with `subscribe_changes`, revision-consistent snapshot export, and `activate_panel_with_completion`; activation callbacks now receive `(outcome, &mut App)` after owner state is released.
...
- `open-gpui-wgpu` now owns one private generation-bound surface lifecycle per native window. Zero extent suspends acquisition instead of configuring `1x1`, ordinary resize configures at a render-safe point without an unbounded shared-device wait, usable suboptimal frames present before reconfiguration, and unrecoverable surface loss publishes a renderer-neutral rejection for only that window.
...
- `open-gpui-ui-components` now lets `VirtualizedList` hosts own the scroll handle, materialize stable keys through `VirtualizedListState::materialization_target_for_key*`, complete nested physical reveal through `VirtualizedList::bring_key_into_view`, and keep nested actions contained without taking row ownership away from the list.
...
- `open-gpui-devtools` now projects Table behavior through a pre-capture allowlist with session-scoped opaque table, column, and row ordinals plus derived cell ids; raw ids, values, diagnostic source identities, selectors, and deterministic hashes are excluded from live, history, diff, Inspector, export, artifact, report, and Gallery fixture output, while sanitized diagnostic kinds and counts remain observable.
