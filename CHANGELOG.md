...
- `open-gpui-docking` moved `{DockController, DockControllerBuilder}` out of the crate root and `prelude`; use `DockSurface::builder` for common setup or import controller types from `open_gpui_docking::model` for low-level integrations.
- `DockSurface::from_controller`, raw host/controller accessors, direct host-options builder injection, and the typed `DockHost` primary-window return are no longer part of the common `DockSurface` facade; use facade builder methods and the erased `AnyWindowHandle` primary-window return, with low-level access under `open_gpui_docking::model` or `open_gpui_docking::runtime`.
- `open-gpui-motion` moved low-level scalar/runtime/spec/model/policy imports such as `MotionExecutionPlan`, `MotionPolicyInput`, and `validate_motion_policy` under `open_gpui_motion::advanced`; ordinary component motion should use the root `MotionTransition`, `MotionScalarRun`, `MotionProgressRun`, `MotionFrameDriver`, and sequence facades.
...
- `open-gpui-devtools` diagnostics now include a stable `code`; replace `SnapshotDiagnostic { probe_id, message }` struct literals with `SnapshotDiagnostic::new(...)` or `SnapshotDiagnostic::collection_failed(...)`. `ProbeId::new(...)` now sanitizes ids at creation/deserialization, `SnapshotKind::as_label()` returns `Cow<'_, str>` while sanitizing custom labels for UI rows and JSON export, `SnapshotNode::new`, `SnapshotNode::with_payload`, and `SnapshotRedactionSummary` now sanitize DTO output channels instead of preserving raw strings, adapter payload helpers stay behind the `adapters` facade, and framework adapter enum labels are exported as kebab-case JSON strings.
- `open-gpui-devtools` command keybinding and keymap helpers now use the same snapshot helper naming as the rest of the crate; replace `command_keybinding_projection_envelope`, `command_keybinding_projection_probe`, `command_keymap_resolution_envelope`, and `command_keymap_resolution_probe` with `command_keybinding_projection_snapshot_envelope`, `command_keybinding_projection_snapshot_probe`, `command_keymap_resolution_snapshot_envelope`, and `command_keymap_resolution_snapshot_probe`.
