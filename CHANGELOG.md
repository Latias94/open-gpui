...
- Official overlays now share one per-window `WindowOverlayRuntime` for lifecycle, topmost input, modal focus, and restoration. `OverlayFocusTarget`, the public `FocusScopeRuntime` constructor, `OverlayLayerHost`, and family-local focus/dismissal tails are removed; `OverlayPresence::from_parts` is fallible, `FocusRestoreInput::ancestor_last_targets` is renamed, and custom adapters use the low-level window registration API only when an official component is insufficient.
