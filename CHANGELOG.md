...
- `open-gpui` adds `PlatformWindowHit::ProvisionalPassThrough` for exact-generation native routing through gated provisional windows. Update exhaustive matches and custom point-hit stacks to emit only a front-to-back provisional prefix followed by one registered or opaque terminal; keep or return `PlatformWindowHitStack::Unavailable` when complete coverage cannot be proven.
