...
fn command_sample_from_local(
    ...
    let state = CommandState::resolve(CommandStateRequest {
        ...
        query_mode: CommandQueryMode::Uncontrolled,
        selection_mode,
        selected_value: selected.map(str::to_owned),
        selected_values: selected_values.clone(),
        active_value: active.map(str::to_owned),
        loading_state: loading,
        empty_label: "No results".to_string(),
        dialog_title: dialog.then_some("Command palette".to_string()),
        dialog_description: dialog.then_some("Run a workspace command".to_string()),
        data_source: CommandStateDataSource::local(groups.iter().cloned(), items.iter().cloned()),
        outside_press_policy: OutsidePressPolicy::DismissAndConsume,
...
fn command_sample_from_snapshot(
    ...
    let state = CommandState::resolve(CommandStateRequest {
        ...
        dialog_title: dialog.then_some("Command palette".to_string()),
        dialog_description: dialog.then_some("Run a workspace command".to_string()),
        data_source: CommandStateDataSource::snapshot(snapshot.clone()),
        outside_press_policy: OutsidePressPolicy::DismissAndConsume,
        escape_key_policy: EscapeKeyPolicy::Dismiss,
        initial_focus_intent: InitialFocusIntent::FirstFocusable,
        focus_restore_intent: FocusRestoreIntent::Trigger,
