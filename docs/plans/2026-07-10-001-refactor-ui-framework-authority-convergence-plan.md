...
R28. Give each facade-managed `DockSurface` one explicit, generation-bound window session with a unique anchor, owned committed and provisional viewports, and `Vacant/Closed -> Opening -> Active -> ShuttingDown -> Closed` lifecycle. Embedded, primary-anchor, and managed-viewport hosts have distinct private roles; render cannot infer or activate an anchor. Primary creation is authorized by its exact `Opening` token; after commit, only work carrying the exact current `Active` lease is admitted, including managed viewport opens/restores, registration, scene publication, activation, drag, route, mutation, and observation. Pre-commit creation/map/draw failure, synchronous close during creation, duplicate open, application shutdown, post-commit presentation failure, and stale generation callbacks settle explicitly without rolling an Active session back into Opening. Closing the current anchor freezes new work, cancels active drag and pending/provisional viewport work, then force-closes a deduplicated snapshot of only that surface's owned windows after all runtime borrows are released, with dependent windows terminal before an anchor that remains live. `ShuttingDown` reaches `Closed` only when the exact anchor is terminal, or confirmed absent after App shutdown clears the registry, the current-generation runtime registry is empty, and every snapshotted close ticket is terminal or post-clear confirmed absent; reopen is rejected before that point. App shutdown converges even when GPUI clears the registry without per-window close observers. Shutdown is idempotent, bypasses per-viewport `Prevent` and `MergeBack` policies, never unconditionally quits the application, and cannot be triggered by stale anchor generations or implicit low-level hosts.

R29. Route native cross-window Dock drag from the capture-owning source through one generation-bound active-drag authority. Each source move or terminal edge produces one immutable captured-drag fact containing the source full `WindowId`, drag generation, ingress sequence, button or terminal kind, current global screen point as `Point<DevicePixels>`, a point-scoped classified front-to-back native hit stack, source scale factor, and point-covering geometry as `Bounds<DevicePixels>` in the same physical desktop space. The hit stack distinguishes complete generation-bearing registered application windows from opaque barriers without leaking raw handles; only destroyed, minimized, or demonstrably invisible entries may be skipped. Dock resolves that fact using the exact surface/session and host-scene generations. Underlay resolution explicitly passes through only the current provisional/session-gated window that may cover the point. A current same-surface host is eligible; a host from another Dock surface is a typed forbidden target with rejected preview; every ordinary, foreign-process, unregistered, stale-scene, or otherwise unknown visible top-level window is an opaque barrier that yields desktop tear-off rather than allowing a hidden Dock host below it to receive preview or drop. Windows must report physical global screen geometry and preserve exact registered sibling IDs before child-only normalization. Dock converts the physical screen point directly into the chosen target's local logical `Pixels` using that target's current client origin and scale; it never subtracts logical global origins measured under different monitor scale factors. `MouseUp` locks its point, hit stack, candidate, and generations exactly once; later first-presentation work may revalidate liveness at that locked point but may not read a later mouse position. Source callbacks record facts, release their `Window`/Dock borrows, and then deliver them in R26 ingress order; they never inject raw input into a target HWND or transfer native capture. A normal `MouseUp` is terminal release authority, while its following capture-change notification is cleanup only.

R30. Start opening and reusing one provisional tear-off viewport before release once a drag crosses the live-undock threshold. Reserve its exact U26 provisional-opening token before `open_window`; when synchronous opening returns a committed full `WindowId`, register an explicit U26-owned `ProvisionalViewport` role and close ticket before admitting presentation, routing, or promotion facts. It is not a committed Dock viewport, host-scene, activation target, or durable `DockGraph` entry until promotion. It owns one generation-bound payload-subtree presentation lease and immutable source presentation snapshot covering item, tabs, and floating payloads while the committed graph and revision remain unchanged. Lease activation immediately revokes the old source payload subtree's input, focus, and accessibility eligibility, then requires `SourceProxyCommitted(source WindowId, source frame generation, lease generation)` before the hidden provisional mounts the real payload. The source retains one stable semantic/focus proxy rather than a duplicate panel subtree, and the provisional becomes the only live content renderer only after that committed source-frame barrier.

The provisional is created with the ordinary detached viewport's final role-aware native profile, normally a peer top-level window with no initial activation, so same-HWND promotion never requires a native owner or capability transition. A generation-bound GPUI window-session interaction gate is synchronously readable by every owning backend without borrowing `App`: it suppresses provisional native activation, pointer/key/text/IME dispatch, focus, route eligibility, and AccessKit ownership while continuing paint, resize, close, and lifecycle facts. Suppressed user input and accessibility actions are rejected rather than replayed after promotion; only framework-owned promotion/focus completion work may carry the promotion generation.

The live-undock state is orthogonal: transport terminal state, provisional readiness, current route feedback, presentation-lease location, and release latch evolve independently. Hidden create advances through non-empty accepted/submitted/present observation and no-activate show into a continuously visible provisional. Entering a valid host or forbidden surface does not hide that HWND: the target and provisional render generation-matched accepted or rejected route feedback, with alpha only an optional visual refinement. Repeated movement repositions the same window. A `MouseUp` that races readiness records exactly one locked `ReleasePending` fact from R29; a host outcome may retire an unseen provisional, while a desktop release waits a bounded, injectable first-presentation deadline to promote that same HWND or restores the still-live source on timeout/failure. Direct provisional terminal close or renderer failure is `Unavailable`: a current host result may still commit, but desktop release restores the source.

Every fallible native effect and promotion preflight completes while the provisional remains gated. A `PreparedPromotion` clones and validates the complete next Dock workspace plus role, registry, lease, semantic/focus, and gate transition before the final App transaction. That final closure performs only an infallible state swap, publishes the durable revision, and schedules post-commit effects; graph validation or preparation failure therefore occurs before durable mutation and can restore the exact live source. Desktop promotion retains the same HWND; release to another valid host transfers and commits there before activating the target host; return or cancellation restores source presentation and focus. Source deactivation or shutdown never steals focus back. Repeated release/cancel callbacks, rejection, Escape, capture loss, source/payload/anchor close, stale generation, creation/presentation failure, or promotion-preparation failure settle one terminal result without graph mutation, duplicate semantic ownership, or leaked windows. A generation-bound `PresentationShutdownTicket` stops new draw/present work, drains or retires current submissions, releases backend surface-bound resources, and records a typed quiesced acknowledgement before native window destruction may publish terminal state.
...
21. Two independent Dock surfaces each open an anchor and detached viewports. An embedded host never becomes an anchor, and the first managed surface admits no viewport or scene/activation fact until synchronous `App::open_window` returns a committed full `WindowId` and its `Opening` token validates into an exact `Active` lease. Closing that anchor during an active drag enters shutdown once, cancels its opening/provisional and pending work, force-closes only its runtime-owned dependent windows despite `Prevent` or `MergeBack`, and leaves the second surface usable. It remains `ShuttingDown` until the exact anchor is terminal/absent, its runtime is empty, and every close ticket is terminal/absent; reopening is rejected until `Closed`, after which a new generation admits no stale create/close callback. Direct App shutdown with all window kinds alive also converges after registry clear without relying on per-window close observers.
22. With Win32 capture held by the source HWND, the pointer crosses a second viewport that receives no raw mouse move. The source-owned route passes through only its exact provisional role, converts the current global screen point into the target's current coordinate space, publishes the target preview before release, and commits the drop exactly once. `MouseUp` locks its ingress sequence, point, hit stack, candidate, and generations; a delayed first-presentation task may revalidate only liveness at that locked point and never substitute a later cursor position. A host from another Dock surface shows a rejected preview and cancels on release. An ordinary or foreign opaque top-level window over a Dock host is a desktop barrier, so the hidden host receives neither preview nor drop. Capture loss, Escape, source close, or anchor shutdown clears every preview and pending task without using the last valid target. Closing only the current target clears that target's preview and pending work while retaining the source-owned drag; the next movement or release resolves again from the current screen point.
23. A user drags an item, tab set, or floating payload onto the desktop. After crossing the threshold, the source first commits a generation-matched semantic/focus proxy frame; only then does one U26-owned provisional mount the real payload and normally become visible with a non-empty presented frame before button-up. Its final peer-top-level native profile remains exact-session-gated and the durable graph/revision remain unchanged. Repeated movement repositions rather than recreates it; entering a host or rejected foreign surface keeps the same provisional visible and projects current accepted/rejected feedback into both surfaces. Its gate rejects provisional native activation, pointer/key/text/IME, focus, route, and AccessKit actions rather than replaying them after promotion. If button-up races source-proxy commit, create, show, or first-present, one locked `ReleasePending` result preserves suppression: a host result retires the unseen provisional, while desktop waits a bounded interval to promote that same non-empty window or restores the source on failure. Normal desktop release prepares and validates the complete next state while suppressed, then one App transaction performs an infallible swap, removes the gate, and only afterwards activates the new viewport and restores the previously focused live payload descendant. Release over another valid host transfers and commits there before its activation host receives focus; return or cancellation restores source presentation and focus before destroying the provisional. Recoverable creation, renderer, surface, presentation, optional native-effect, or promotion-preparation failure restores only while the source generation remains live. Source deactivation or shutdown cancels without stealing focus or attempting restoration into a dead source, and every provisional close quiesces renderer/surface work before native terminal state.
...
KTD28. **DockSurface owns a window session, not application exit.** A private session authority, distinct from the viewport facade, owns `Vacant -> Opening -> Active -> ShuttingDown -> Closed`, a monotonic generation, exact anchor identity, shutdown cause, and terminal ticket snapshot. `open_primary_window` first reserves an `Opening` anchor token, then calls synchronous `App::open_window` inside the current App transaction. Registry commit returns the full `WindowId`; a short owner borrow validates that ID and token before transitioning to the one current `Active` generation. Native callbacks raised during creation queue until commit or rollback and never activate the session themselves; a primary host carries `Opening(token)` until commit and may draw without registering, then atomically becomes `Active(lease)`. Pre-commit create/map/initial-draw or before-visibility presentation failure settles `Opening -> Closed`; once the full ID commits, later presentation failure settles `Active -> ShuttingDown(PresentationFailed) -> Closed` through forced teardown rather than reviving or rolling back the consumed token. Every post-commit facade-owned operation carries the opaque exact-generation lease so stale work cannot pass a new session through optional-value equality. The primary installs a close-request guard: the first ordinary request freezes the session and returns `false`, dependents drain outside all borrows, and convergence explicitly removes the still-live anchor last; direct native destruction enters the same path from its exact native terminal. Surface-owned forced teardown bypasses individual viewport close policies and is isolated from other surfaces. `Closed` means the exact anchor and every snapshot ticket have reached native terminal, the current-generation runtime registry is empty, or App shutdown has confirmed the remaining exact windows absent only after registry clear; reopen is rejected until convergence. A later generation remains isolated from delayed prior-generation callbacks. Native owner relationships remain presentation hints, and last-window application exit stays GPUI policy rather than a Dock `quit` shortcut.

KTD29. **Captured transport is source-owned, point-scoped, and post-borrow.** The native capture owner alone samples a screen point and classified point hit stack, then publishes one immutable, generation-bound fact after source `Window` and Dock borrows release. The stack preserves exact registered sibling `WindowId` values and opaque native barriers; it is not an application-only window list and does not leak HWNDs. Dock may pass through only its exact current provisional, resolves current same-surface hosts without target raw input, rejects foreign-surface hosts, and stops at every other opaque top-level. `MouseUp` latches its route fact and is release authority; later capture-change is cleanup, while later readiness work may revalidate only liveness at the latched point. GPUI owns generic capture transport and terminal ordering, the platform owns classified native observations, and Dock owns route interpretation. ImGui informs source capture and platform-window staging; Open GPUI retains typed barriers, full generations, and retained-graph transactions.

KTD30. **Live provisional presentation is a gated payload lease, not an early graph commit.** A `DockLiveUndockSession` owns the payload-subtree presentation lease, immutable source snapshot, provisional role/handle reference, route feedback, readiness, release latch, and compensating saga; the viewport runtime remains the sole handle registry and DockSurface remains the sole shutdown authority. The lease covers item, tabs, and floating payloads, immediately revokes stale source-subtree interaction/semantic eligibility, waits for a generation-matched `SourceProxyCommitted` frame, then exposes the real content only in the provisional while leaving one source semantic/focus proxy. The provisional is registered only as a U26-owned provisional role until final promotion. It remains hidden until the source proxy barrier and a non-empty provisional present both complete, then stays visible across desktop, accepted-host, and rejected-host feedback; target/provisional feedback is projected from the same route generation instead of hiding and reopening the HWND. A backend-readable interaction gate rejects all provisional user input and accessibility actions without altering final native lifetime capabilities. `ReleasePending` retains a locked `MouseUp` fact while readiness settles. Every fallible effect and complete next-state validation produces one `PreparedPromotion` while gated; one final App transaction performs only an infallible state swap, transfers the lease and semantic/focus ownership, removes the interaction gate, and publishes the revision. Same-HWND desktop promotion, host transfer, cancellation, failure, source/anchor shutdown, and stale callbacks each converge once without durable pre-commit mutation, duplicate semantics, or focus theft. Provisional teardown requires a generation-bound renderer/surface quiescence acknowledgement before native terminal state.
...
DockSurface window session
  ...
  Vacant/Closed -> Opening(anchor token; draw-only primary role)
      ...
      `-> anchor close
             ...
             -> close dependent windows outside borrows, then live anchor
             -> exact anchor terminal/absent + snapshot tickets terminal/absent
...
readiness:      hidden Opening -> payload mounted -> non-empty presented -> visible / Unavailable
route feedback: desktop <-> accepted host <-> rejected foreign/barrier
...
promotion: prepare complete next state -> infallible App swap removes gate -> revision -> activation
...
| DockSurface owner | session phase, exact anchor token/generation, opaque active lease, terminal ticket snapshot, and shutdown authority |
| Dock viewport runtime | sole session-lineage-tagged registry of committed, opening, and provisional window handles |
| Dock live-undock session | locked release facts, current route feedback, source-proxy and provisional readiness, provisional role/handle reference, payload presentation lease, source semantic/focus proxy, prepared promotion, presentation-shutdown ticket, and release/compensation saga |
...
U24 Platform Event Safety -> U25 Window Appearance/Owner -> U26 Dock Window Session
                                                             |
                                                             v
U27 Captured Native Drag Routing -> U29 Live Provisional Promotion -> U28 Owning-Platform Evidence -> final gate
```
...
**Provisional viewports leak, flash, or duplicate semantic ownership.** Repeated motion, failed first paint, target switching, an early `MouseUp`, delayed promotion, or shutdown can leave windows alive, expose an empty shell, settle twice, or render one payload as two focus/accessibility owners. Mitigation: U29 allows one U26-owned provisional role, one payload-subtree presentation lease, and one source semantic/focus proxy per drag/session generation; revokes the stale source subtree and waits for a committed proxy frame before mounting provisional content; separates readiness, route, lease, and release-latch axes; keeps the non-empty provisional continuously visible; bounds early desktop release with an injectable clock; rejects gated user input rather than replaying it; prepares and validates the complete next state before an infallible swap removes the gate; and requires renderer/surface quiescence before HWND terminal.
...
The references inform behavior and ownership only. Their package layouts, APIs, and runtimes are not copied wholesale. The repository has no `docs/solutions/` corpus to inherit for App borrow reentrancy, cross-HWND capture routing, or Dock window-session teardown; the code, native reproduction evidence, prior plans, and exact ImGui clone are the grounding sources for U24-U29. ImGui contributes source capture, staged platform-window presentation, same-window reuse, and the evidence that default native parentage harms z-order inference. Open GPUI deliberately retains its fail-closed opaque-barrier hit stack, typed generations, retained graph transactions, and explicit session teardown rather than copying ImGui's global moving-window state or foreign-window fallback.
...
- Review confirms non-stealing initial show plus later activation, explicit permanent non-activation, owner generation safety, first-presentation observability, a native profile compatible with later same-window promotion, and honest optional per-backend flag capability.
...
- Split close dispatch from native convergence. `claim_close_dispatch` is exactly-once intent only, while the matching native `Closed` event settles terminal state. A failed exact-handle update remains pending because neither dispatch failure nor logical registry removal proves native destruction. Apply effects only after every owner/controller/runtime/entity borrow is released. Forced teardown bypasses `Prevent` and `MergeBack`, suppresses ordinary graph merge/focus restoration, closes dependent windows before an anchor that is still live, then admits `Closed` only when the anchor and all snapshot tickets are native-terminal and the current-generation runtime is empty. Only App shutdown may confirm remaining exact windows absent after registry clear.
...
- Integrate App shutdown explicitly. The synchronous pre-clear portion of `on_app_quit` freezes the session, snapshots ownership, releases borrows, and applies forced close without the ordinary deferred close applicator. It also queues one dedicated convergence effect that runs under GPUI's existing `window_registry::clear -> flush_effects` shutdown order and marks only the remaining exact tickets `ConfirmedAbsent(AppShutdown)`. Test and document this lifecycle rather than waiting for `on_window_closed`, which registry clear does not emit.
...
- App-shutdown tests cover both `Opening` and a fully `Active` session with anchor and committed viewports. They prove pre-clear freeze/close, post-clear confirmed-absent settlement without per-window close callbacks, no permanent `ShuttingDown`, and no cross-surface or unmanaged-runtime corruption. U29 adds the provisional-alive shutdown case.
...
- `crates/gpui_windows/src/platform.rs`
- `crates/gpui_windows/src/window.rs`
- `crates/gpui_windows/src/native_test_support.rs`
...
- Extend GPUI's active-drag boundary with one typed captured-pointer transport. The source capture owner samples a global `Point<DevicePixels>`, point-covering `Bounds<DevicePixels>`, source scale, button or terminal kind, and a point-scoped classified native hit stack under one drag generation and ingress sequence. The transport does not inject arbitrary raw input into another window or transfer native capture.
- Add one private AppCell-owned `NativeCapturedDragOutbox` at the input transaction boundary. U24's native ingress passes the original, non-forgeable application sequence into the `WindowUpdateTransaction`; the source callback may only append an immutable drag fact to that outbox. After the window transaction and outer `AppRefMut` are released, AppCell drains the outbox to the exact generation-bound Dock consumer while preserving the original sequence. Move facts may coalesce only under the declared drag generation, while release, cancel, capture-change, close, and consumer replacement remain non-coalescing barriers. A missing, stale, or unregistered consumer retires the fact explicitly instead of redispatching it under a new sequence.
- Add a point-scoped platform hit-stack capability beside the legacy application-window list. Its entries distinguish `RegisteredApplication(full WindowId)` from `OpaqueBarrier` without exposing raw HWNDs. A missing or incomplete stack is a typed route-capability gap, never permission to look through an unknown top-level. The narrow consumer is Dock routing; it is not a general cross-window event API.
- Implement the Windows stack as a front-to-back enumeration at the sampled physical screen point. Preserve ordinary, unregistered, and foreign-process top-level windows as opaque barriers. Match a registered top-level HWND to its exact full `WindowId` before any child-only root normalization; never normalize through `GW_OWNER`, which would collapse registered siblings into the anchor. Skip only destroyed, minimized, or demonstrably invisible entries. Correct Windows facts/capabilities to expose global screen geometry and observed live position mutation only where the backend actually supports it, including negative coordinates and mixed DPI. The point-stack contract remains in Win32 physical desktop coordinates; do not derive it by dividing each window's global origin into a different logical coordinate space.
- Resolve every Dock route from the fact's full `WindowId`, exact U26 `Active` lease, physical point/coverage, target scale and client origin, current committed host-scene generation, and input eligibility. Convert from the physical screen point to local logical `Pixels` only after selecting the target, using that target's current scale. A same-surface current host is eligible; a foreign-surface registered host is `ForbiddenTarget`; a registered window with no current eligible scene and every opaque entry terminates lookup as a desktop barrier. Until U29 adds the provisional role, nothing is transparent. A later U29 provisional may be passed through only when its exact role, drag, and session generations match.
- Route preview facts to the target host through Dock's existing interaction authority without assuming that HWND received `MouseMove`. Source and target redraw scheduling use U24; one current route owns the preview, and a stale target callback cannot restore an old guide. Existing direct-target listeners remain local behavior only and lose cross-window authority.
- Make `MouseUp`, Escape, `PointerCanceled`, capture change/cancel mode, source deactivation, source close, anchor shutdown, and drag-generation replacement terminal inputs to one source-owned route state machine. `MouseUp` locks the sampled point, hit stack, candidate, host-scene/session/drag generations, and ingress sequence before generic drag/capture cleanup. A later `WM_CAPTURECHANGED` from normal release may clean up but cannot cancel or replace that locked release. U29 may revalidate liveness at the locked point only; it may not read a later pointer position.
- Bind every route candidate, preview, release fact, and post-borrow effect to the exact U26 `Active` lease. Session shutdown cancels matching U27 transport before U26 dependent-window teardown; delayed G1 facts cannot join G2 even when optional anchor fields match. Embedded and unmanaged hosts participate only through their declared low-level ownership contract.
- Replace the 16 ms outside-release poll as route authority with source transport terminal facts. Preserve the existing release-time desktop tear-off only as a temporary U27 destination after the locked route has resolved; U29 replaces that fallback with pre-release provisional presentation. No direct native open/show/move/close, target update, or refresh occurs while a source Window, Dock entity, controller, or runtime `RefCell` borrow is held; those operations use U24's post-borrow effect/command boundary and revalidate their generation on return.
...
- Point-stack tests cover overlapping windows from the same and different Dock surfaces, ordinary application and foreign-process barriers, visible registered windows without eligible host scenes, registered siblings retaining distinct full `WindowId` values despite native ownership, child-only normalization, destroyed/minimized/invisible entries, exact generation replacement, negative physical screen coordinates, and mixed-DPI monitor boundaries. The mixed-DPI case proves target-local conversion uses the selected target's scale and never subtracts per-window logical global origins.
- Route eligibility tests cover resize/move/DPI change, host-scene/session replacement, target close just before release, foreign-surface forbidden feedback/cancellation, opaque barriers, stale local points, and last-preview rejection. An opaque window over a Dock host proves the hidden host receives neither preview nor drop.
- Locked-release tests move the pointer after `MouseUp` but before delayed consumer work or readiness completion and prove the original point, stack, candidate, and generations remain authority. A normal release followed by `WM_CAPTURECHANGED` proves capture cleanup cannot convert release into cancellation; Escape/cancel remain ordered barriers.
- Borrow-boundary tests synchronously pump move/up/close/capture-change callbacks while source input, target updates, and post-borrow effects interleave. They prove the AppCell outbox preserves the original ingress sequence across release of `WindowUpdateTransaction` and outer `AppRefMut`, no nested `RefCell` borrow or newly sequenced redispatch occurs, no terminal is dropped, and no native open/show/move/close runs under a live Dock/runtime borrow. Missing/stale consumer, consumer replacement, App shutdown, and queued close barriers each retire or deliver the exact fact once.
...
- Multi-surface and shutdown tests prove route/session isolation, target-close re-resolution, and source transport cancellation before U26 dependent-window cleanup.
...
- Delete the assumption that a target HWND receives raw mouse movement under source capture, the outside-release polling path as route authority, app-only stack filtering that makes opaque ordinary/foreign windows transparent, and last-valid-preview release fallback.
...
- Review confirms one source-owned captured transport, AppCell outbox preservation of the original ingress sequence across both borrow boundaries, full-`WindowId` stack classification, fail-closed opaque barriers, global coordinate honesty, foreign-surface rejection, `MouseUp` latching, no poll/last-preview authority, complete terminal cleanup, and no native effect under a live source/Dock/runtime borrow.

### U29. Add Live Provisional Presentation And Same-HWND Promotion

**Outcome**

After a Dock payload crosses the live-undock threshold, one U26-owned provisional viewport renders the live payload before normal release without committing durable topology. It stays visible and gated while routing changes, then is atomically promoted, transferred, or compensated exactly once.
...
- Add an explicit private `ProvisionalViewport` ownership and host role. Reserve a provisional-opening token under the exact U26 `Active` lease before opening the HWND; after synchronous `open_window` returns the committed full `WindowId`, bind the role and native terminal ticket before admitting presentation, route, or promotion facts. Include that ticket in the session's shutdown snapshot and forced close/terminal convergence. The role is intentionally absent from committed Dock viewport registration, host-scene routing, activation, persistence, and durable graph topology until the final promotion transaction converts the same full `WindowId` into a committed viewport role.
...
- Define a payload-subtree presentation lease for every `DockDragPayload` form: item, tabs, and floating subtree. Lease activation immediately marks the old source payload subtree ineligible for input, focus, and AccessKit actions even if its previous frame is still displayed. The source then commits an immutable semantic/focus proxy frame and reports `SourceProxyCommitted` under the exact source window, frame, payload, and lease generations. Only after that barrier may the hidden provisional mount the real payload and become its sole live renderer. Source close, failed proxy draw, generation mismatch, removal, graph mutation, or incompatible application action either rejects against the live lease or cancels it before mutating; final preparation revalidates the exact source payload snapshot and generation.
- Add a generation-bound GPUI window-session interaction gate whose immutable snapshot can be read synchronously by owning platform callbacks without borrowing `App`. Create its closed handle before `open_window`, carry it through the reserved-window/creation record and root closure, and bind it to the committed full `WindowId` only after opening succeeds; construction callbacks and initial root work therefore cannot observe an ungated provisional. The gate permits paint, resize, close, presentation, and lifecycle facts, but rejects provisional native activation, pointer/key/text/IME dispatch, focus, Dock route eligibility, and AccessKit subtree ownership. On Windows the gate participates in `WM_MOUSEACTIVATE`, hit testing, input dispatch, focus/activation, and accessibility paths; root rendering uses the matching inert presentation scope. It is not a `PointerInput` or activation-policy mutation: the final detached native profile remains installed from creation. Suppressed user input and accessibility actions are discarded, never replayed after promotion; only internal focus/promotion completion work may be queued with the promotion generation.
- Decouple logical DockSurface lineage from native owner selection. Remove U26's automatic `transient_for` assignment for every facade-managed viewport. Ordinary committed and provisional detached viewports are peer top-level by default; an explicit application-requested owner survives only when it matches the final role and is supported. The same HWND is therefore valid before and after desktop promotion. U26 session teardown remains the only lifecycle cascade authority.
- Create the provisional hidden with the final no-initial-activation profile and pre-created interaction gate. Its root remains an empty gated shell until the matching source proxy frame commits; only then bind/mount the source snapshot under the payload lease and admit its accepted/submitted/non-empty-present observations. Show only after a current non-empty present. Once shown, it remains visible through desktop, host-preview, and rejected feedback; do not add a general live visibility mutation or emulate it with minimize, close, or pointer-input capability changes. U27's observed live placement updates move the one visible provisional. Route feedback is rendered from one generation into the target host and provisional; alpha is optional polish rather than correctness.
- Model live undock as orthogonal axes rather than a linear enum: transport terminal state; source-proxy readiness (`AwaitingSourceProxy`, committed, failed); provisional readiness (`Opening`, payload mounted, non-empty visible, `Unavailable`, terminal); current route (`Desktop`, valid host, forbidden host, barrier); lease location (source proxy, gated provisional, committed destination); release latch (`None`, exact locked `ReleasePending`, consumed); and presentation shutdown (none, quiescing ticket, acknowledged). A first-present fact can therefore arrive while the route is host/foreign, and target movement cannot destroy readiness or overwrite the locked release fact.
- When U27 reports any release before provisional readiness settles, retain exactly one `ReleasePending` with its locked point/stack/candidate/generations. A valid host or source result retires the unseen provisional and commits or restores immediately; only a desktop result waits a bounded, injectable deadline (default 500 ms) for that same HWND's non-empty first presentation, then promotes it or compensates to the exact still-live source. Motion facts may coalesce only within the active drag generation; release, cancel, close, presentation terminal, and promotion results are ordered non-coalescing barriers.
- Define one compensating promotion/transfer saga. Complete every fallible native effect, renderer/surface preflight, route check, source payload validation, next-graph construction, and role/registry/lease/semantic/gate validation while the provisional is gated, producing an immutable `PreparedPromotion`. The final App closure performs only an infallible swap of those prepared states, publishes the single durable revision, and schedules destination activation through U19; it cannot return a graph or partial-handoff error. Desktop promotion preserves the same HWND; a valid other-host release transfers to that host and destroys the provisional only after the transfer is committed; local return/cancellation restore only a live source. Preparation failure compensates under exact generations, removes route feedback/proxy/gate, and destroys or retires the provisional without an interactive half-promoted window. U19 emits no durable revision before the final swap.
- Source, payload, or anchor shutdown, drag replacement, Escape, and capture loss settle the matching live session once before U26 dependent-window teardown. Closing only the current non-source target clears that target's preview and waits for the next U27 fact instead of terminating the source-owned drag. Direct provisional native close or renderer/device/surface failure marks readiness `Unavailable`: a current valid-host release may still commit, while a desktop result restores only the still-live source. Stale callbacks retire without changing current state. A dead source never receives restored focus or presentation. Every provisional close first claims an exact-generation `PresentationShutdownTicket`, prevents new draw/present submission, drains or invalidates outstanding work, releases DirectX swapchain/DirectComposition and WGPU surface-bound resources through a backend quiesce hook, and publishes a typed acknowledgement. Only that acknowledgement permits native destruction and terminal publication; U28 verifies the production ordering rather than supplying it.

**Test scenarios**

- Lease tests cover item, tabs, and floating payloads. They hold the provisional payload unmounted until `SourceProxyCommitted`, immediately reject stale source hit/focus/AccessKit actions, and prove source-proxy draw failure or generation mismatch cancels without showing a provisional. After the barrier and before promotion, each has one live content renderer, one source semantic/focus proxy, no duplicate AccessKit subtree, no graph revision, and a conflict table for close/remove/reorder/application activation against the lease.
- Gate tests prove a provisional can paint, resize, report first presentation, close, and receive terminal lifecycle facts while `WM_MOUSEACTIVATE`, hit test/input, key/text/IME, focus, Dock targeting, and AccessKit actions are rejected. After one successful promotion they become enabled under the committed destination; no suppressed user action is replayed.
- Presentation tests cover pre-created closed gate, hidden empty root, source-proxy commit, source snapshot binding, payload mount, first accepted frame, submitted present, non-empty present, no-activate show, continuous visible movement, host/rejected feedback projection, same-HWND reuse, renderer/device/surface failure, and no empty shell or overlapping source/provisional payload frame. Entering a host never hides or destroys the provisional, and repeated host-desktop-host movement never opens a second window.
...
- Orthogonal-state tests combine opening with accepted/foreign/barrier routes, first-present during host feedback, route changes during readiness, delayed feedback, and a target close that clears only its preview then re-resolves from the next U27 fact.
- Early-release tests lock `MouseUp`, pump capture-change/close/first-present callbacks during creation and show, advance only a fake/injectable clock, and prove exactly one result: host/source retirement, same-HWND desktop promotion, or restoration to the exact live source. Later pointer motion cannot alter the locked release point or target.
- Saga tests cover valid other host, valid source host, desktop, opaque barrier desktop, forbidden foreign surface, target generation replacement, optional native-effect failure, provisional creation/renderer/surface/first-presentation failure, promotion preparation failure, next-graph validation failure, source payload mutation, compensation failure, and rollback to the exact source generation. They prove the final prepared-state swap is infallible and publishes exactly one revision; every pre-swap failure asserts route/proxy/gate/task cleanup, one AccessKit owner, focus rules, and no focus steal on source deactivation/shutdown.
- Presentation-shutdown tests prove an exact ticket blocks new draw/present work, late generations cannot quiesce a replacement, DirectX and WGPU surface-bound resources acknowledge release before native terminal, repeated close is idempotent, and no late present targets a destroyed window.
- Multi-surface/App-shutdown tests prove only the owning surface's provisional is cancelled/closed, direct provisional terminal events cannot leak a handle or late present, and active registry clear converges even without per-window close observers.
...
- Review confirms provisional-role isolation, exact U26 lease/ticket ownership, source-proxy committed barrier before payload mount, continuous non-empty pre-release presentation, one payload renderer and source proxy, backend-readable gate/no replay, peer-top-level default and explicit owner policy, locked early release, same-HWND promotion, complete `PreparedPromotion` validation before an infallible final swap, presentation-shutdown acknowledgement before HWND terminal, graph/revision atomicity, and exhaustive cleanup.
...
- Expose typed, metadata-only test observations for callback envelope/disposition, window and session generation, source capture owner, screen point and classified hit stack, locked release facts, source-proxy frame commit, first accepted frame, first non-empty presentation, native visibility, interaction-gate state, native activation, peer/explicit-owner relationship, `PresentationShutdownTicket` claim/quiescence acknowledgement, and terminal shutdown. Assertions use those facts rather than parsing one generic log or treating visibility as paint.
...
- Drive two real HWNDs with capture retained by the source. Before a normal release, assert the target Dock preview commits a presentation generation without target raw mouse input. Outside every host, assert the provisional HWND is visible, contains a non-empty presented frame, follows the pointer, defaults to peer top-level final ownership, and remains exact-session-gated for input/focus/activation/routing/accessibility. Entering a valid or forbidden host must retain that same visible HWND and project route feedback, not hide it. Separately force `MouseUp` during create/show/first-present, move the system cursor afterwards, and prove the locked `ReleasePending` fact settles once without a nested App borrow or an empty promoted shell.
...
- Cover the full release and cancellation matrix through actual native messages: valid cross-window drop, desktop same-HWND promotion, source return, Escape, capture loss, target-close re-resolution, source/payload/anchor terminal close, source-proxy commit failure, provisional creation or presentation failure, promotion preparation/next-state validation failure, repeated close, stale generation, and suppressed provisional input/AccessKit actions. Assert that no failure is injectable after the final prepared-state swap begins. Bound every worker with a timeout and guaranteed process-level cleanup so failures cannot leave CI windows/processes alive.
- Verify ordinary detached creation does not steal foreground focus but later click and programmatic activation work. Assert the default peer top-level relationship, explicit supported owner behavior, and no automatic DockSurface-native owner coupling, while separately proving explicit surface teardown.
...
- Start two Dock surfaces in one process, close one real anchor during active/provisional work, and assert all and only its dependent HWNDs become terminal before the live anchor and its session reaches `Closed`. Close the remaining anchor and assert the application reaches its normal last-window/process termination condition without example-level `cx.quit`. In a separate worker, invoke App shutdown while an active anchor, committed viewports, and provisional are all live; registry-clear confirmation must converge even without per-window close observers.
...
- Real two-HWND tests cover OS-routed source-only captured move/up through system pointer injection, actual recipient and `GetCapture` observations, classified point stacks, underlay resolution through only an exact visible provisional role, opaque ordinary/foreign-window barriers over a Dock host, distinct exact IDs for peer siblings and explicit owned siblings, target preview before release, continuous provisional presentation/feedback before release, locked `MouseUp` facts, coordinate/DPI changes, gated-input rejection, activation/focus/AccessKit handoff after no-focus appearance, and owner/z-order facts.
- Real failure tests cover capture cancel/deactivation, `MouseUp`/capture-change/close nested during source-proxy commit or provisional opening, source/target/payload/anchor destruction, pre-commit and post-commit anchor presentation failure with their distinct terminal paths, failed provisional first presentation, failed promotion preparation/next-state validation, infallible final swap, presentation-ticket acknowledgement before HWND terminal, runtime close-observer reentry, and repeated/late close with no double borrow, graph corruption, or leaked HWND.
- A source-HWND-close-during-capture test observes exactly one pointer cancellation and terminal `GetCapture() == NULL`, with no stale drag, preview, or provisional presentation surviving the close.
- Real surface tests cover `Prevent`/`MergeBack` under ordinary close, forced bypass under anchor shutdown, two-surface isolation, dependent-terminal-before-anchor ordering, delayed child terminal-close observation, active App-shutdown registry clear without close callbacks, rejected reopen while the old anchor/runtime/tickets remain live or registered, explicit reopen generation after convergence, absence of overlapping old/new generation HWNDs, and final process convergence.
...
- Review confirms the tests cross the actual WndProc/capture/point-stack/present/lifetime boundaries, every reported regression has a native negative-then-positive scenario, normal and locked early release are distinguished, opaque occlusion cannot target hidden hosts, the source-proxy barrier prevents overlapping payload ownership, gated provisional input is never replayed, peer/explicit-owner roles are distinguished, prepared promotion has no fallible final-swap branch, the production presentation-shutdown ticket precedes HWND terminal, App shutdown and anchor-last convergence are proven, simulated evidence is labeled honestly, and no test worker can leak a process or HWND after failure.
...
- Native drag correctness requires OS-routed source-only captured input, actual capture/recipient observation, a physical-device-pixel point-scoped classified hit stack, full registered sibling IDs, target-scale local conversion across mixed DPI, opaque ordinary/foreign barriers, typed foreign-surface rejection, post-borrow route consumption, and a locked `MouseUp` fact that later cursor motion or capture-change cannot replace. Live-undock correctness additionally requires one U26-owned provisional role, one item/tabs/floating payload lease, a generation-matched committed source semantic/focus proxy before payload mount, orthogonal readiness/route/lease/release/shutdown state, a backend-readable interaction gate with no user-action replay, a continuously visible non-empty peer-top-level provisional before normal `MouseUp`, bounded early-release settlement, complete `PreparedPromotion` validation under the gate, an infallible final graph/committed-registry/lease/semantic swap plus gate removal, destination activation/focus completion, a generation-bound presentation-shutdown acknowledgement before HWND terminal, compensation, and complete cleanup. Direct target event injection, direct source message injection alone, `IsWindowVisible` alone, or a release-created window cannot satisfy the claim.
...
- U1-U20 and U24-U29 are implemented in dependency order. Candidate U21-U23 remain separate follow-on epics. Every unit's focused local tests pass before its commit; platform-owned CI evidence that requires a committed revision passes before that unit and the plan are declared complete.
...
- Creation-time focus, lifetime activation/click/input, permanent non-activation, accepted/submitted/non-empty presentation, and owner/transient relationships are independent facts. An ordinary detached viewport does not steal initial focus but later activates; DockSurface lineage never implies native ownership; ordinary and provisional detached viewports are peer top-level by default, while an explicit supported owner remains available. A provisional starts with the final native lifetime profile but remains generation-gated until same-window promotion; unsupported ownership and optional flags remain explicit.
- Every facade-managed DockSurface has a private session authority distinct from the renamed viewport facade, explicit host roles, an `Opening` token, one exact active anchor generation/lease, public read-only status, and typed admission/open outcomes. Synchronous window creation must return a committed full `WindowId` before token validation activates the session; construction callbacks and first render cannot activate or register it. The surface owner stores phase/anchor/shutdown/tickets, the session-lineage runtime solely stores window handles, and opening failure or synchronous close settles every tentative mapping before viewport admission. Anchor shutdown freezes new work, cancels provisional/pending work, force-closes dependent windows before a live anchor outside all borrows, bypasses `Prevent`/`MergeBack`, leaves other surfaces intact, and reaches `Closed` only after exact anchor/runtime/ticket convergence. Active App shutdown converges across registry clear without ordinary close observers; premature reopen, `cx.quit`, stale callbacks, and native owner lifetime assumptions are absent.
- Native captured Dock drag routes from the source using immutable physical global-screen and point hit-stack facts without target raw mouse delivery. Delivery occurs after source/Dock borrows release. Target-local logical coordinates are derived only with the selected target's current scale, including across mixed-DPI monitors. `MouseUp` locks its point, stack, candidate, and generations; later cursor motion and normal capture-change cleanup cannot replace it. Routing passes through only the exact current provisional role, stops at ordinary/foreign/unknown opaque barriers, types foreign-surface hosts as rejected targets rather than desktop fallback, clears one drag generation on every cancel/close path, and never drops onto stale, provisional, or occluded hosts.
- One Dock live-undock session exclusively owns the provisional role/reference, item/tabs/floating payload lease, immutable source snapshot, source semantic/focus proxy, readiness/route/lease/release/shutdown axes, prepared promotion, presentation-shutdown ticket, and compensation saga. Threshold crossing revokes stale source interaction/semantic eligibility, commits the source proxy frame, then mounts the payload in one peer-top-level reusable viewport before release; after non-empty show it remains visible across desktop/host/rejected feedback while the graph/revision stay unchanged. The backend-readable generation gate rejects activation, input, IME, focus, routing, and AccessKit actions without replay. A locked early `MouseUp` enters one bounded `ReleasePending` settlement and cannot promote an empty shell or settle twice. Every fallible promotion step and complete next-state validation finishes under the gate; one final App transaction performs only an infallible prepared-state swap, removes the gate, publishes one revision, and then schedules destination activation. Creation, renderer/surface, presentation, native preflight, promotion preparation/next-state validation, cancellation, or shutdown failure restores only a live source, never steals focus on deactivation/shutdown, acknowledges renderer/surface quiescence before HWND terminal, and leaks no window or duplicate semantic owner.
- Windows real-HWND and subprocess gates cross WndProc, system-injected pointer routing, actual capture/recipient and point-stack observation, locked release, continuous non-empty presentation, gate rejection, activation/focus/AccessKit handoff, peer/explicit-owner roles, renderer-before-HWND terminal close, and process-lifetime boundaries. TestPlatform coverage is labeled as simulation and cannot claim native end-to-end credit.
...
| --- | --- | --- |
| R1-R2 | all units; preservation gates in U10-U20 and U24-U29 | import/dependency scans and preserved-module focused tests |
...
| R15 | each breaking unit; U11 audits prior surfaces; U12-U20 and U24-U29 own their migrations | same-unit migration docs/Gallery/examples/DevTools updates and final residual scan |
...
| R26 | U24, U27-U28 | AppCell-owned ingress, application sequence and merge/FIFO/barrier matrix, synchronous-query snapshots, exact App-idle hybrid input disposition with zero busy entrances, closed post-borrow command FIFO, already-borrowed asynchronous replay, full-`WindowId` commit/rollback isolation, borrow-free subordinate effects, accepted-or-reinvalidated frame tests, captured transport post-borrow delivery, and real nested native-message evidence |
| R27 | U25, U29, U28 | independent appearance/activation/input/owner contract, peer-top-level default plus explicit owner, deletion of live focus-on-appear mutation, accepted/submitted/non-empty presentation generations, no-focus-then-activate native flow, owner capability tests, and same-window session promotion |
| R28 | U26, U29, U28 | host-role separation, synchronous registry-commit activation, exact active lease, provisional close-ticket ownership, sole session-lineage runtime registry, typed public status/outcomes, borrow-free forced close snapshot, dependent-before-anchor and App-shutdown terminal convergence before `Closed`/reopen, stale generations, multi-surface isolation, real HWND/process evidence |
| R29 | U27, U29, U28 | OS-routed source-only captured move/up, actual capture/recipient facts, physical-device-pixel point-scoped classified hit-stack resolution through only the exact provisional role, target-scale local conversion across mixed DPI, opaque ordinary/foreign barriers, exact session/host eligibility, foreign-surface rejection, locked `MouseUp` facts, cancel/close barriers, and real two-HWND preview/drop evidence |
| R30 | U29, U28 | one live-undock session, payload-subtree presentation lease, generation-matched committed source proxy before payload mount, orthogonal readiness/route/lease/release/shutdown state, normal continuous non-empty pre-release viewport plus bounded locked early-release settlement, provisional role, fail-closed gate/no user replay, complete `PreparedPromotion` validation plus an infallible graph/registry/lease/semantic swap and gate removal, destination focus completion, exhaustive release/cancel/failure table, generation-bound `PresentationShutdownTicket` acknowledgement before HWND terminal, and leak-free native teardown |

### Priority Rationale

- **P0 correctness:** U1 and U2. They fix data corruption risk and make a critical user-facing authority observable.
- **P1 interaction/runtime:** U3-U6, U12-U17, U19-U20, and U24-U29. They resolve modal, focus, accessibility, activation, geometry, presentation, announcement, anchoring, reveal, clipping, Dock application ownership, platform-event loss, native window semantics, session teardown, and real multi-viewport drag with the largest user impact. The later units were discovered or promoted by substrate, real-consumer, and owning-platform audits; their execution position does not reduce their severity.
...
plan. Candidate U21-U23 are post-plan scheduling handles only; U12-U20 and U24-U29 must not add public
...
- Before U29 live content: require Dock/GPUI/accessibility/platform review of the single live-undock session, exact U26 provisional role/ticket, item/tabs/floating payload lease, generation-matched source-proxy commit barrier, orthogonal readiness/route/lease/release/shutdown axes, backend-readable interaction gate with no user-input replay, peer-top-level default, continuous visible provisional feedback, locked `ReleasePending`, exhaustive release/focus/accessibility decisions, complete prepared-state validation before an infallible final swap, and production renderer/surface quiescence before native teardown.
- After U29: require native-input/Dock review of exact provisional-only transparency, one content renderer and one semantic owner after the source-proxy barrier, no empty shell, visible non-empty presentation before normal release, no host-transition hide/show, same-HWND promotion, one-window reuse, payload conflict policy, destination activation/focus/AccessKit ownership, `PreparedPromotion` plus infallible graph/registry/lease/semantic swap and gate removal, U26 shutdown integration, and `PresentationShutdownTicket` acknowledgement before HWND terminal.
- After U28: require evidence review proving the designated tests use U24's real-HWND support, system-level pointer injection without a selected receiver, actual WndProc/capture/point-stack/present/lifetime boundaries, locked release, gated input rejection, peer/explicit-owner roles, renderer-before-HWND teardown, delayed-close/reopen exclusion, every reported regression's failure path, deterministic worker cleanup, and honest VisualTest labeling.
