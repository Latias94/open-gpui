...
- Implement Windows `Platform::window_stack` as a real front-to-back enumeration of registered application top-level HWNDs, mapped to complete generation-bearing `WindowId` values. Normalize descendant and owned native HWND hits to their registered top-level application window, exclude destroyed and foreign-process handles, and leave Dock surface/session, provisional, ordinary non-Dock, and input-eligibility filtering to the typed consumer rather than silently truncating the native stack.
...
- Windows stack tests cover overlapping windows from the same and different Dock surfaces, ordinary application windows, foreign-process windows, descendant/owned HWND normalization, destroyed handles, and exact full-`WindowId` replacement without stale aliasing.
