...
- native pointer capture can route a Dock drag across real windows without the target HWND receiving raw mouse messages, a provisional viewport is visibly presented before normal release, an early release settles through one bounded pending state, and release or cancellation commits exactly one graph transition;
...
22. With Win32 capture held by the source HWND, the pointer crosses a second viewport that receives no raw mouse move. The source-owned route passes through only its provisional HWND, converts the current screen point into the target's current coordinate space, publishes the target preview before release, revalidates the same surface/session and host generation on button-up, and commits the drop exactly once. A host from another Dock surface shows a rejected preview and cancels on release. An ordinary or foreign opaque top-level window over a Dock host is a desktop barrier, so the hidden host receives neither preview nor drop. Capture loss, Escape, source close, or anchor shutdown clears every preview and pending task without using the last valid target. Closing only the current target clears that target's preview and pending work while retaining the source-owned drag; the next movement or release resolves again from the current screen point.
23. A user drags a tab onto the desktop. After crossing the threshold and normally before button-up, one runtime-registered provisional viewport is visible with a non-empty presented frame while its final native lifetime profile is exact-session-suppressed, one source semantic/focus proxy remains authoritative, and the durable graph/revision remain unchanged. Repeated movement repositions rather than recreates it; entering a host hides the provisional presentation without destroying its reusable HWND and shows only that host's guide. If button-up races creation/show/first-present, one `ReleasePending` result preserves suppression: a host result retires the unseen provisional, while desktop waits a bounded interval to promote that same non-empty window or restores the source on failure. Normal desktop release completes fallible promotion work while suppressed, then one App transaction commits graph/registry/lease ownership and removes the gate before activating the new viewport and restoring the previously focused live panel descendant. Release over another valid host transfers and commits there before its activation host receives focus; return or cancellation restores source presentation and focus before destroying the provisional. Recoverable creation, renderer, surface, presentation, optional native-effect, or commit failure restores only while the source generation remains live. Source deactivation or shutdown cancels without stealing focus or attempting restoration into a dead source.
...
KTD28. **DockSurface owns a window session, not application exit.** A private session authority, distinct from the viewport facade, owns `Vacant -> Opening -> Active -> ShuttingDown -> Closed`, a monotonic generation, exact anchor identity, shutdown cause, and terminal ticket snapshot. `open_primary_window` first reserves an `Opening` anchor token, then calls synchronous `App::open_window` inside the current App transaction. Registry commit returns the full `WindowId`; a short owner borrow validates that ID and token before transitioning to the one current `Active` generation. Native callbacks raised during creation queue until commit or rollback and never activate the session themselves; a primary host carries `Opening(token)` until commit and may draw without registering, then atomically becomes `Active(lease)`. Pre-commit create/map/initial-draw or before-visibility presentation failure settles `Opening -> Closed`; once the full ID commits, later presentation failure settles `Active -> ShuttingDown(PresentationFailed) -> Closed` through forced teardown rather than reviving or rolling back the consumed token. Every post-commit facade-owned operation carries the opaque exact-generation lease so stale work cannot pass a new session through optional-value equality. The primary installs a close-request guard: the first ordinary request freezes the session and returns `false`, dependents drain outside all borrows, and convergence explicitly removes the still-live anchor last; direct native destruction is the confirmed-absent fallback. Surface-owned forced teardown bypasses individual viewport close policies and is isolated from other surfaces. `Closed` means the exact anchor is terminal or confirmed absent, the current-generation runtime registry is empty, and every snapshot ticket is terminal or confirmed absent; reopen is rejected until convergence. App shutdown has an explicit pre-clear/confirmed-absent path because registry clear does not imply ordinary close observers. A later generation remains isolated from delayed prior-generation callbacks. Native owner relationships remain presentation hints, and last-window application exit stays GPUI policy rather than a Dock `quit` shortcut.

KTD29. **Live undock and durable graph commit are separate.** The native capture owner transports the current screen point plus a classified native z-order stack. Dock passes through only its own provisional/session-suppressed window, resolves a current same-surface host without target HWND input, rejects a foreign-surface host, and treats an opaque ordinary/foreign top-level window as a desktop barrier rather than reaching a hidden Dock host beneath it. Dock interaction/controller state exclusively owns the panel presentation lease, stable source semantic/focus proxy, and graph saga; the viewport runtime solely stores window handles, DockSurface owns session/shutdown authority over that registry, and GPUI reports only native frame/presentation facts. Crossing the threshold starts one provisional before release and reuses it. If `MouseUp` arrives while it is still `Opening`, one generation-bound `ReleasePending` fact preserves suppression and source authority until a host result retires the unseen provisional or a bounded desktop first-presentation attempt promotes the same HWND or compensates. Release revalidates the target and completes every fallible promotion preflight or native effect while the provisional remains session-suppressed. One final App transaction atomically commits graph ownership, viewport registration, presentation lease, semantic/focus ownership, and gate removal; generation-bound queued input then drains after commit or retires after rollback. Promotion activates the committed destination and restores the valid pre-drag descendant through the destination activation host, while cancellation restores only a live source and source deactivation/shutdown never steals focus. ImGui anchors the source-capture and staged-platform-window behavior; Open GPUI retains retained graph transactions, typed barriers, terminal outcomes, and rollback.
...
DockSurface window session
  ...
  Vacant/Closed -> Opening(anchor token; draw-only primary role)
      ...
      |                         |-> early MouseUp: ReleasePending -> promote/compensate
...
ReleasePending -> one early release retained under suppression until host settlement or bounded first presentation
...
Each facade-managed `DockSurface` owns one explicit anchor session and shutdown authority, while the viewport runtime remains the sole registry of committed and provisional handles for that session. An ordinary anchor close request is held until that surface drains its dependents; direct destruction and App shutdown still converge idempotently, and another DockSurface remains independent.
...
- Add a private `window_session` authority that owns only `Vacant/Opening/Active/ShuttingDown/Closed`, a monotonic session generation, exact opening/anchor identity, shutdown cause, and terminal ticket snapshot. The viewport runtime remains the sole handle authority for committed, opening, tear-off, and provisional windows; the owner receives a teardown snapshot but never mirrors those handles.
...
- Install the surface close-request guard on the primary during construction. An ordinary request for the exact current anchor transitions to `ShuttingDown`, prepares shutdown, and returns `false` so native destruction cannot overtake dependents. One runtime operation validates and freezes the active lease, cancels drag/preview/tear-off/focus/activation/mutation work, retires the generation's mappings, and builds a deduplicated exact-window snapshot across committed adapters, opening ownership, tear-off reservations, and provisional handles. It returns pure typed effects after terminally settling pending activation/completion work; when dependent tickets converge, one effect explicitly removes the still-live exact anchor. Repeated requests remain vetoed/idempotent, while programmatic/native destruction that bypasses the guard confirms the anchor absent and continues cleanup.
- Split close dispatch from native convergence. `claim_close_dispatch` is exactly-once intent only; `observe_closed` records a matching terminal callback; and a failed exact-handle update becomes `confirm_absent`, not a swallowed error. Apply effects only after every owner/controller/runtime/entity borrow is released. Forced teardown bypasses `Prevent` and `MergeBack`, suppresses ordinary graph merge/focus restoration, closes dependent windows before an anchor that is still live, then admits `Closed` only when the anchor is terminal/absent, the current-generation runtime is empty, and all snapshot tickets are terminal/absent.
...
- Failure-order tests cover the OS destroying a child first, anchor close during provisional creation or promotion, tear-off commit failure, both close-observer orders, callback-opened work, delayed terminal observation for one old HWND, failed exact-handle update becoming confirmed absent, rejected reopen while the anchor/registry/tickets remain incomplete, and successful reopen only after deterministic convergence.
- App-shutdown tests cover both `Opening` and a fully `Active` session with anchor, committed viewports, and a provisional alive. They prove pre-clear freeze/close, post-clear confirmed-absent settlement without per-window close callbacks, no permanent `ShuttingDown`, and no cross-surface or unmanaged-runtime corruption.
...
- Extend GPUI's active-drag boundary with a typed captured-pointer transport. The source capture owner samples the current screen point and a classified native z-order stack plus committed window geometry, preserves source button/cancel ordering, and delivers a generation-bound drag fact to the registered consumer. Entries distinguish exact registered application windows from opaque native barriers without exposing raw handles. The transport does not inject arbitrary raw GPUI input into another window or transfer native capture.
- Implement Windows `Platform::window_stack` as a real front-to-back enumeration that preserves opaque ordinary, unregistered, and foreign-process top-level barriers instead of filtering them away. Map registered application windows to complete generation-bearing `WindowId` values; match an owned registered secondary HWND to its own exact ID before any root-owner normalization so siblings do not collapse into the anchor. Skip only destroyed, minimized, or demonstrably stale entries. Dock surface/session, provisional transparency, host role, and input eligibility remain typed consumer decisions.
- Resolve each candidate from the full `WindowId`, exact current `Active` session lease, scale factor, current screen-to-window conversion, committed host-scene generation, and input eligibility. Continue through the one current provisional/session-suppressed window and through explicitly stale/destroyed entries. A current same-surface host is eligible; a registered host from another surface is `ForbiddenTarget` and owns rejected feedback; an opaque ordinary/foreign/unregistered barrier terminates host lookup and yields the current desktop tear-off route. Never pass through an opaque barrier to preview or drop onto a hidden Dock host.
...
- Treat `MouseUp`, Escape, `PointerCanceled`, native capture change/cancel mode, source deactivation, source close, anchor shutdown, and drag-generation replacement as terminal inputs to one idempotent cancellation/release state machine. `MouseUp` always releases source capture and ends movement; when presentation is still `Opening`, it retains only the suppressed provisional, source proxy/lease, and one `ReleasePending` fact until bounded settlement. Final commit/cancel/failure clears preview, pending task, presentation lease, proxy, and provisional exactly once; later capture-change or close callbacks cannot settle again.
...
- Bind every route candidate, provisional reservation, presentation fact, and release/compensation effect to the exact U26 `Active` lease. Session shutdown cancels the matching U27 work; delayed G1 work cannot join G2 even when both optional anchor fields are absent. Embedded/unmanaged hosts participate only through their explicitly declared low-level ownership contract, never by borrowing a facade session.
- Once the pointer crosses the threshold, advance the retained live-undock session from prepared payload through provisional opening before `MouseUp`, first presentation, visible movement, promotion/transfer, committed, cancelled, and superseded states. One drag generation may own at most one provisional viewport; repeated movement and host/desktop transitions reposition and show/hide that same handle through U20 rather than opening another.
...
- Define the release decision table:
  ...
  - `MouseUp` while the provisional is still `Opening` records exactly one generation-bound `ReleasePending`; a valid host/source result may retire the unseen provisional before its normal commit, while a desktop result waits a bounded interval for that same HWND's non-empty first presentation and promotes it, or restores the source and destroys it on timeout/failure;
  ...
  - an opaque ordinary/foreign/unregistered top-level is a desktop barrier: it prevents any host below from receiving preview/drop while retaining or promoting the current provisional at the pointer;
...
- Route eligibility tests cover target resize/move/DPI change, host-scene/session generation replacement, target close just before release, foreign-surface forbidden preview/cancellation, provisional transparency, opaque ordinary/foreign barriers, stale local points, and last-preview rejection. Host-to-desktop-to-host movement proves the same provisional never obscures the restored host route, while an opaque window over a Dock host proves the hidden host receives neither preview nor drop.
- Windows stack tests cover overlapping windows from the same and different Dock surfaces, ordinary application and foreign-process barriers, registered owned siblings retaining distinct full `WindowId` values, descendant normalization, destroyed/minimized entries, and exact generation replacement without stale aliasing.
...
- Provisional tests cover every `Opening`/`ReleasePending`/`DesktopVisible`/`HostPreview`/`Rejected`/`Unavailable`/terminal transition, one-window reuse across repeated motion and target transitions, no empty shell, final lifetime native capability plus exact-session suppression, native visibility plus non-empty first presentation before normal `MouseUp`, live position updates, one source semantic/focus proxy, no duplicate focus/accessibility/input ownership, and no early graph revision.
- Release-table tests cover other host, source host, same-window desktop promotion, opaque-barrier desktop promotion, foreign-surface forbidden target, invalid target, stale target re-resolution, optional native flag failure, provisional creation/renderer/surface/first-presentation failure, promotion preflight failure, graph commit failure, queued input/focus/accessibility during delayed promotion, compensation failure handling, and rollback to the exact source generation. Early-release tests pump `MouseUp`, capture loss, and close callbacks during create/show/first-present, proving one pending release, no nested `RefCell` borrow, bounded desktop settlement, and exactly one final result. Each success and cancellation path asserts final activation completion, focused descendant, one AccessKit owner, and proxy removal; source deactivation/shutdown asserts no focus steal.
...
- Delete the assumption that a target HWND receives raw mouse movement under source capture, the outside-release polling path as route authority, release-only viewport creation, and filtering that makes opaque ordinary/foreign windows transparent to hidden Dock hosts.
- Delete duplicate source-window and provisional semantic presentations, parallel outside-release/tear-off/interaction route state superseded by `DockLiveUndockSession`, last-valid-preview release fallback, hovered-top-window-only routing that cannot distinguish the provisional from an opaque barrier, and any provisional window not owned by the current surface/drag/session generation.
...
- Review confirms non-overlapping state ownership, exact-session admission, capture-owner transport, z-ordered provisional transparency plus opaque barriers, foreign-surface rejection, early-release settlement, complete terminal cleanup, one presentation lease and semantic/focus proxy, explicit presentation states, visible non-empty viewport before normal release, gate-preserving preflight plus atomic final handoff, exhaustive release/focus/accessibility outcomes, graph atomicity, and no duplicate semantic owner.
...
- Drive two real HWNDs with capture retained by the source. Before a normal release, assert the target Dock preview commits a presentation generation without target raw mouse input. Outside every host, assert the provisional HWND is visible, contains a non-empty presented frame, follows the pointer, has the final native lifetime profile, and remains exact-session-suppressed for input/focus/activation/routing/accessibility. Separately force `MouseUp` during create/show/first-present and prove `ReleasePending` settles once without a nested App borrow or an empty promoted shell.
...
- Real two-HWND tests cover OS-routed source-only captured move/up through system pointer injection, actual recipient and `GetCapture` observations, underlay resolution through a visible provisional, opaque ordinary/foreign-window barriers over a Dock host, distinct exact IDs for sibling top-level viewports sharing one native owner, target preview before release, provisional presentation and every display state before release, coordinate/DPI changes, activation/focus/AccessKit handoff after no-focus appearance, and owner/z-order facts.
- Real failure tests cover capture cancel/deactivation, `MouseUp`/capture-change/close nested during provisional opening, source/target/anchor destruction, pre-commit and post-commit anchor presentation failure with their distinct terminal paths, failed provisional first presentation, failed promotion/commit, runtime close-observer reentry, and repeated/late close with no double borrow, graph corruption, or leaked HWND.
...
- Review confirms the tests cross the actual WndProc/capture/present/lifetime boundaries, every reported regression has a native negative-then-positive scenario, normal and early release are distinguished, opaque occlusion cannot target hidden hosts, App shutdown and anchor-last convergence are proven, simulated evidence is labeled honestly, and no test worker can leak a process or HWND after failure.
...
- Native drag correctness requires OS-routed source-only captured input, actual capture/recipient observation, classified z-order resolution through only the provisional, opaque ordinary/foreign barriers, current screen-to-target conversion, typed foreign-surface rejection, exact-session and release-time revalidation, one live-undock session/presentation lease and source semantic/focus proxy, explicit presentation states including bounded early-release settlement, a visible non-empty provisional frame before normal `MouseUp`, fallible preflight under suppression, atomic graph/registry/lease/semantic/gate handoff, destination activation/focus completion, compensation, and complete terminal cleanup. Direct target event injection, direct source message injection alone, `IsWindowVisible` alone, or a release-created window cannot satisfy the claim.
...
- Native captured Dock drag routes from the source using current screen-space and a classified native z-order stack without target raw mouse delivery. Release revalidates the exact session and target, passes through only the current provisional, stops at ordinary/foreign opaque barriers, types foreign-surface hosts as rejected targets rather than desktop fallback, clears one drag generation on every cancel/close path, and never drops onto stale, provisional, or occluded hosts.
- One Dock live-undock session exclusively owns route, provisional reference, presentation lease, source semantic/focus proxy, and compensation saga. Its explicit display states never expose an empty or duplicate shell; threshold crossing starts one reusable viewport before release, and normal drag proves a visible non-empty frame before `MouseUp` while the graph/revision remain unchanged. An early `MouseUp` enters one bounded `ReleasePending` settlement and cannot promote an empty shell or settle twice. Every fallible promotion step completes under exact-session suppression; one final App transaction atomically commits graph, registry, lease, semantic/focus ownership, and gate removal before destination activation. Creation, renderer/surface, presentation, native preflight, graph commit, cancellation, or shutdown failure restores only a live source, never steals focus on deactivation/shutdown, and leaks no window or duplicate semantic owner.
...
| R28 | U26, U28 | host-role separation, synchronous registry-commit activation, exact active lease, sole session-lineage runtime registry, typed public status/outcomes, borrow-free forced close snapshot, dependent-before-anchor and App-shutdown terminal convergence before `Closed`/reopen, stale generations, multi-surface isolation, real HWND/process evidence |
| R29 | U27-U28 | OS-routed source-only captured move/up, actual capture/recipient facts, classified z-order resolution through only provisional windows, opaque ordinary/foreign barriers, exact session plus screen/DPI/host eligibility, foreign-surface rejection, release revalidation, cancel/close barriers, and real two-HWND preview/drop evidence |
| R30 | U27-U28 | one live-undock session, presentation lease, and semantic/focus proxy, explicit states including `ReleasePending`, normal non-empty pre-release viewport plus bounded early-release settlement, fallible preflight under suppression, atomic graph/registry/lease/semantic/gate handoff, destination focus completion, exhaustive release/cancel/failure table, and leak-free native teardown |
...
- Before U27 live content: require Dock/GPUI/accessibility review of the single live-undock session, exact U26 lease, controller-owned presentation lease, source semantic/focus proxy, explicit provisional states including `ReleasePending`, session suppression, classified native z-order with provisional transparency and opaque ordinary/foreign barriers, foreign-surface rejection, exhaustive release/focus/accessibility decisions, gate-preserving preflight, atomic final handoff, and renderer/surface failure rollback.
- After U27: require native-input/Dock review of source-capture transport, target raw-event independence, exact sibling `WindowId` preservation, provisional-only transparency, opaque occlusion, release revalidation, bounded early-release settlement, all terminal cancellation paths, visible non-empty presentation before normal release, same-window promotion, one-window reuse, destination activation/focus/AccessKit ownership, and graph/registry/lease/gate atomicity.
