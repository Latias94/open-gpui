...
U24 Platform Event Safety -> U25 Window Appearance/Owner -> U26 Dock Window Session
                                                             |
                                                             v
U27 Captured Native Drag Routing -> U29 Live Provisional Promotion -> U28 Owning-Platform Evidence -> U30 Accepted-Frame Activation -> final gate
```
...
- Add a real mixed-DPI live-undock scenario on a runner with two monitors reporting distinct effective DPI values. Drive the production captured source move, visible non-empty provisional presentation, locked release, and same-HWND promotion with the release point on the higher-DPI display. The backend prepares the final client bounds with target-monitor DPI, accepts any synchronous `WM_DPICHANGED`, performs at most one deterministic target-side inset correction under the same placement generation, and publishes exact final client bounds, display identity, and scale for the same HWND. Keep a separate generic placement unit test for the lower-level API. Missing distinct-DPI capacity is a runner capability failure, not a skipped product assertion or a tolerance-based pass.
...
| R26 | U24, U27-U28, U30 | AppCell-owned ingress, application sequence and merge/FIFO/barrier matrix, synchronous-query snapshots, exact App-idle hybrid input disposition with zero busy entrances, closed post-borrow command FIFO, already-borrowed asynchronous replay, full-`WindowId` commit/rollback isolation, borrow-free subordinate effects, accepted-or-reinvalidated frame tests, captured transport post-borrow delivery, real nested native-message evidence, and accepted-frame activation publication/rollback evidence |
...
| R30 | U29, U28 | one live-undock session, pending-open cancellation/bind-and-compensate, payload-subtree presentation lease, generation-matched committed source proxy before payload mount, frozen-source visual continuity, orthogonal readiness/route/lease/placement/release/destination-semantics/shutdown state, private generation-bound non-activating reveal with native hit transparency and typed visibility/Z-order evidence, normal continuous non-empty pre-release viewport plus bounded locked early-release placement settlement, terminal unavailable release, provisional role, fail-closed gate/no user replay, complete `PreparedPromotion` validation, KTD32 `Abortable -> ForwardOnly` exact-receipt settlement, KTD33 transaction-level staging retention and one final Graph sweep, destination-semantics acknowledgement only after exact Graph, placement, presentation, and lease proofs, committed-destination recovery on Graph supersession or post-boundary/pre-semantics terminal failure, exhaustive release/cancel/failure table, generation-bound `PresentationShutdownTicket` acknowledgement before HWND terminal and App registry clear, and leak-free native teardown |
...
- Before completion: simplify-code pass, structured code review, supported-platform renderer/window-mutation/multi-viewport evidence, full Verification Contract after U30, and release-doc audit.
