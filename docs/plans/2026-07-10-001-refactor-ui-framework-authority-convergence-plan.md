...
- A transient degraded refresh schedules one coalesced event-loop retry chain with a bounded backoff interval while retaining the previous publication. It continues at that bounded interval until success, replacement, or shutdown cancels it, so exact display authority cannot remain permanently degraded merely because one topology collection raced an operating-system transition. The retry timer may wake collection but never manufacture geometry progress or placement acceptance.
...
- Make native creation and the first explicit client-geometry request one retained placement transaction with typed `Pending`, `Presenting`, and `Presented` phases. The `Presenting` phase remains observable while a native show call synchronously pumps callbacks; a reentrant request retargets or supersedes the retained authority rather than escaping as an unrelated live mutation. A request accepted before initial map/reveal therefore cannot be overtaken by an already queued initial-presentation command or silently rolled back after it reported success. Decorated X11 creation remains pending until the matching post-map frame-extents/configure observation either proves the requested client geometry or performs one generation-bound correction, while a later user move or resize supersedes that correction.
...
- On Windows, retain exact client readback and deterministic same-generation correction. One private non-client policy distinguishes a native caption, a custom titlebar, and an undecorated frame; it is the single source for creation styles, `WM_NCCALCSIZE`, client-to-outer projection, and hit testing. The policy and observed client geometry are authoritative when User32 normalizes overlapped style bits, so an undecorated contract is not inferred from `WS_CAPTION` absence alone. Windowed initial presentation first settles hidden client geometry, then uses a move-free/size-free `SetWindowPos` reveal rather than `ShowWindow` restore semantics, and commits only after a visible native readback still matches the same client-placement intent. A visible provisional placement that fails post-mutation validation hides before restoring geometry and z-order, revalidates the complete prior authority, and only then performs a move-free/size-free reveal; rejected native geometry may never flash or publish callbacks as committed placement. On X11 or another asynchronous frame-extents backend, keep the transaction pending across map/decoration events, reconcile only the matching generation after native readback, and let a later unrelated user move win. A backend without that proof declares global client bounds, target-display DPI, and exact placement unsupported instead of inheriting Windows credit.
- On macOS, keep Cocoa point-space client geometry authoritative: creation, ordinary bounds, restore placement, `content_size`, display identity, and backing scale must come from one coherent `contentRect`/`contentView` observation that also retains the exact detached display fact from that generation. `display()`, bounds, scale, and state project that same committed value even while a newer topology publication awaits window re-observation. `NSWindow.frame` is an outer-frame projection only and must never be persisted as `WindowBounds` or reused as a client origin. A titlebar, transparent titlebar, fullscreen, or Retina-screen transition starts a generation-bound observation. Each topology publication records the newest generation the window must observe; a coalesced main-thread retry remains armed until that generation commits, a newer generation replaces it, or the window closes. Fullscreen success and failure callbacks, key-window activation edges, and ordinary maximize/restore changes remain pending typed events until the matching complete fact commits, then publish their renderer, focus, accessibility, moved, resize, display-facts, and state effects exactly once. Work-area or other detached display-fact changes wake consumers even when the display identity and window client rect are unchanged. The selected `NSScreen`, detached CG display row, `NSWindow` backing scale, content-view backing conversion, client rect, and window state must agree in one complete sample; otherwise the backend retains the whole previous fact or reports a typed degradation, never a requested-value seed or old-geometry/new-state splice.
...
- Event-driven tests dispatch hidden initial, visible live-route, final-release, ordinary move/resize, DPI, decoration changes, and X11 frame-extents work, then stop timers, pointer input, activation, refresh, expose, and continuous rendering. The matching native callback alone wakes and settles the transaction; stale callbacks cannot wake or commit a replacement generation. Initial-map tests issue a newer client-geometry request before the first presentation command drains and from a synchronous native show callback, proving that only the newer request can become visible and that no accepted mutation is later rolled back by an older presentation attempt.
- Owning-platform tests cover Windows native-caption, undecorated, and custom-titlebar exact client geometry, each policy's requested and User32-observed style plus `WM_NCCALCSIZE` behavior, a move-free/size-free first reveal, visible post-show readback, hidden-before-rollback visible provisional compensation, live decoration changes, negative-origin and two-way mixed-DPI moves, complete display refresh failure, and target display removal during placement. macOS covers decorated and transparent-titlebar content-rect round trips, three consecutive save/reopen cycles without drift, vertical display work-area coordinates, whole-fact retention across unstable fullscreen/titlebar transitions, successful and failed fullscreen transitions with coherent drawable resizing, automatic recovery after transient topology and per-window AppKit sampling failures, key-window gain/loss during degraded topology without lost focus or AccessKit effects, maximize/restore state callbacks exactly once, same-display work-area changes waking consumers, old-generation `display()` consistency, and Retina/non-Retina migration from one coherent `contentRect`/screen/window/view-backing observation. X11 receives an asynchronous initial-map/frame-extents fixture that proves the requested client origin and size, one matching late-extents correction, and a later native user move winning before advertising equivalent exactness; until then it reports window-local bounds and target-display DPI/physical placement as unsupported. Unsupported backends compile and report their narrower capability honestly.
