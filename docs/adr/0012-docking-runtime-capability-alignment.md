...
- `DockPreviewScene` describes target facts and `DockVisualAffordanceScene` owns visual feedback
  semantics: guide boxes, tab insertion, payload tab previews, payload ghosts, route markers,
  divider/corner affordances, focus rings, zoom egress, and rejected state. Render, accessibility,
  motion, diagnostics, and native proof surfaces consume those layers instead of recomputing
  availability.
- `DockTransitionExecutor` owns sampled runtime motion for pane clips, dividers, focus rings, zoom,
  visual affordances, and reduced-motion completion. Visual affordance transitions describe
  tab insertion, payload ghosts, route markers, focus rings, and rejected state; every-frame
  compositor-backed animation for those drag previews is not yet a shipped runtime guarantee.
...
flowchart TB
  ...
  Presentation --> Affordance[DockVisualAffordanceScene]
  Presentation --> A11y[DockAccessibilityScene]
  Presentation --> Zoom[DockZoomScene]
  Affordance --> Render[GPUI visual affordance rendering]
  Affordance --> A11y
  Zoom --> Transition[DockTransitionPlan]
  Affordance --> Transition
  Transition --> Executor[DockTransitionExecutor]
  Runtime[DockViewportRuntimeHandle current facts] --> Affordance
  Affordance --> Status[DockViewportRuntimeStatus diagnostics]
...
- Future docking UI/UX work should add semantic fields to presentation, visual-affordance, transition, or
