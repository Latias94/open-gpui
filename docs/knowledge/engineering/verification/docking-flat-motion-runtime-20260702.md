...
- Overlay runtime: center hover schedules body, insertion, payload tab, and payload ghost samples;
  routed preview tests prove source route marker and target overlay separation. Precise tab
  insertion layers keep current target bounds during hover retargets while guide layers can still
  animate.
