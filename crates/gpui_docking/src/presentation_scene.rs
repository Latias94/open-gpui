...
impl DockPresentationScene {
    ...
    fn collect_split(
        ...
        let geometry = layout.geometry(axis, bounds, session.splitter_handle_size());

        for (index, handle_bounds) in geometry.handle_hit_bounds.iter().copied().enumerate() {
        ...
        for (child, child_bounds) in children.into_iter().zip(geometry.pane_bounds) {
