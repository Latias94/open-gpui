...
fn tab_bar_preview_positions_payload_tab_at_leading_and_middle_slots(cx: &mut TestAppContext) {
    ...
    let leading_index = host
        .update(cx, |host, _| {
            let window_id = host
                .viewport_runtime()
                .window_id_for_space(host.space())
                .expect("host should keep its viewport registration");
            match host
                .viewport_runtime()
                .routed_drop_target_for(host.space(), window_id)
            {
...
fn dragging_tab_to_root_edge_resolves_from_render_leaf_fact_root(cx: &mut TestAppContext) {
    ...
    let target = cx
        .read_entity(&host, |host, _| {
            let window_id = host.viewport_runtime().window_id_for_space(host.space())?;
...
fn class_rejected_edge_hover_renders_rejected_drop_preview_without_commit(cx: &mut TestAppContext) {
    ...
    cx.read_entity(&host, |host, _| {
        ...
        let preview = host
            .viewport_runtime()
            .routed_drop_preview_for(host.space(), window_id)
