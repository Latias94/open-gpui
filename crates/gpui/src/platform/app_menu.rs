...
mod tests {
    ...
    fn test_menu() {
        let menu = Menu::new("App")
            .items(vec![
                crate::MenuItem::action("Action 1", open_gpui::NoAction),
    ...
    fn test_menu_item_builder() {
        ...
        let item = MenuItem::action("Test Action", open_gpui::NoAction);
        assert_eq!(
            match &item {
        ...
        assert!(
            MenuItem::action("Test Action", open_gpui::NoAction)
                .checked(true)
                .is_checked()
        );
        assert!(
