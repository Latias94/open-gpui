use crate::{
    ActiveTooltip, AnyView, App, Bounds, CommittedHitbox, DispatchPhase, Element, ElementId,
            ...
            |interactive_state, window| {
                ...
                if let Some(click_listener) = self.click_listener.take() {
                    if let Ok(mouse_position) =
                        hitbox.window_to_layout_point(window.mouse_position())
                        && let Ok(ix) = text_layout.index_for_position(mouse_position)
                        && self
                            .clickable_ranges
                            .iter()
                    ...
                    if let Some(mouse_down_index) = mouse_down.get() {
                                    ...
                                    && hitbox.is_mouse_event_target(window)
                                {
                                    if let Ok(position) =
                                        ...
                                        && let Ok(mouse_up_index) =
                                            text_layout.index_for_position(position)
                                    {
                                        click_listener(
                                            &clickable_ranges,
                        ...
                        window.on_mouse_event(move |event: &MouseDownEvent, phase, window, _| {
                            if phase == DispatchPhase::Bubble
                                && hitbox.is_mouse_event_target(window)
                                && let Ok(position) = hitbox.window_to_layout_point(event.position)
                ...
                window.on_mouse_event({
                    ...
                    move |event: &MouseMoveEvent, phase, window, cx| {
                        if phase == DispatchPhase::Bubble && hitbox.is_hovered(window) {
                            ...
                            let updated = hitbox
                                .window_to_layout_point(event.position)
                                .ok()
                ...
                if let Some(tooltip_builder) = self.tooltip_builder.clone() {
                    ...
                    let committed_hitbox = interactive_state.committed_tooltip_hitbox.clone();
                    let build_tooltip = Rc::new({
                        ...
                        move |window: &mut Window, cx: &mut App| {
                            committed_hitbox
                                .current(window)
                                .and_then(|hitbox| {
                                    hitbox.window_to_layout_point(window.mouse_position()).ok()
                                })
                                .and_then(|position| text_layout.index_for_position(position).ok())
                    ...
                    // Routed hit testing is unavailable during prepaint, so use the latest
                    // committed source geometry there.
                    let check_is_hovered_during_prepaint = Rc::new({
                        ...
                        move |window: &Window| {
                            committed_hitbox
                                ...
                                .and_then(|hitbox| {
                                    hitbox
                                        .window_to_layout_point(window.mouse_position())
                                        .ok()
                                        .map(|position| (hitbox, position))
                                })
                                .is_some_and(|(hitbox, position)| {
                        ...
                        move |window: &Window| {
                            committed_hitbox
                                ...
                                .is_some_and(|position| {
                                    text_layout.index_for_position(position.1).is_ok()
                                        && position.0.is_hovered(window)
                                })
                                && pending_mouse_down.get().is_none()
                        }
                    });
...
mod tests {
    ...
    impl Render for DelayedTooltipTransformProbe {
        fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
            ...
            div().size_full().child(
                InteractiveText::new("delayed-tooltip-text", StyledText::new("tooltip"))
                    .tooltip(move |_, _, _| {
    ...
    #[crate::test]
    fn delayed_tooltip_uses_latest_committed_transform_geometry(cx: &mut TestAppContext) {
