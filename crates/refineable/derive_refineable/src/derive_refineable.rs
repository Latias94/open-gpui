...
pub fn derive_refineable(input: TokenStream) -> TokenStream {
        ...
        .map(|f| {
            if derives_serialize {
                if is_refineable_field(f) {
                    quote! { #[serde(default, skip_serializing_if = "::open_gpui_refineable::IsEmpty::is_empty")] }
                } else {
    ...
    let r#gen = quote! {
        ...
        impl #impl_generics ::open_gpui_refineable::IsEmpty for #refinement_ident #ty_generics
            #where_clause
        {
