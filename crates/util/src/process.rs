...
/// terminates all processes in the job. This also applies when the Open GPUI
/// process exits for any reason, since the OS closes its handles, so spawned
/// process trees can never outlive their owner.
pub struct Child {
...
impl Child {
    ...
    /// Consumes the child, drains stdout/stderr, and waits for exit.
